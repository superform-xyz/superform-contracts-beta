...
const { stateHandler, superDestination, superRouter, multiTxProcessor } = require("../deployments/fantom.json");
...
async function main() {
    ...
    const tx1 = await superDestinationContract.addVault("0x0EBA9b4844B9381b84376C2D98d3E8E1963F63d4", "2501", {
        gasPrice: GAS_PRICE
    })
    ...
    const bridgeAddress2 = "0x957301825Dc21d4A92919C9E72dC9E6C6a29e7f8"
    ...
    const trustedRemote = hre.ethers.utils.solidityPack(['address', 'address'], ["0xCe029Cc3Fa76F2533313CFB6a2B710De9b37df4E", stateHandler])
    const tx6 = await stateHandlerContract.setTrustedRemote("109", trustedRemote, {
        gasPrice: GAS_PRICE
    })
    ...
    console.log("Processor role set in state handler fantom (vik): ", tx12.hash)
    await tx12.wait()

    const tx13 = await stateHandlerContract.grantRole(PROCESSOR_CONTRACT_ROLE, signer.address, {
        gasPrice: GAS_PRICE
    })
    console.log("Processor role set in state handler fantom (sujith): ", tx13.hash)
    await tx13.wait()
    ...
    console.log("Swapper role set in state handler fantom (sujith): ", tx14.hash)
    await tx14.wait()

    const tx15 = await multiTxProcessorContract.grantRole(SWAPPER_ROLE, vikAddress, {
        gasPrice: GAS_PRICE
    })
    console.log("Swapper role set in state handler fantom (sujith): ", tx15.hash)
    await tx15.wait()
}
