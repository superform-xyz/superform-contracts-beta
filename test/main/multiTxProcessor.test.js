...
describe("multi-tx processor tests:", async() => {
    ...
    before("deploying multi-tx processor", async() => {
        ...
        const MultiTxProcessor = await ethers.getContractFactory("MultiTxProcessor");
    ...
    it("should fail when called by user without swapper role", async() => {
        try {
            await multiTxProcessor.processMultiTx(accounts[0].address, "0x", accounts[0].address, accounts[0].address, 0)
        } catch (err) {
            assert(err)
        }
    })
    ...
    it("add a new swapper role", async() => {
        ...
        expect(await multiTxProcessor.hasRole(role, accounts[1].address)).to.equal(true)
    })
    ...
    it("should be able to process non-native token with swapper role", async() => {
        ...
        await multiTxProcessor.connect(accounts[1]).processTx(1, socketTxData, testToken.address, socket.address, amount)
