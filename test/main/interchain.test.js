...
describe("interchain base tests:", async() => {
    ...
    it("FTM=>BSC: user depositing to a vault requiring swap (stays pending)", async function() {
        try {
            ...
            ++BscStateHandlerCounter;
            await expect(
                BscStateHandler.processPayload(BscStateHandlerCounter)
            ).to.be.revertedWith("tokens not bridged yet");
        } catch (err) {
            console.log(err)
        }
    });
    ...
    it("FTM=>BSC: cross-chain slippage update beyond max slippage", async function() {
        try {
            ...
            let EightTokensE18 = ethers.utils.parseEther("800");

            await BscStateHandler.updateState(BscStateHandlerCounter, [EightTokensE18]);
        } catch (err) {
            console.log(err)
                /// should error;
            assert.include(err.message, "revert", 'State Handler: Slippage Out Of Bounds');
        }
    });
    ...
    it("FTM=>BSC: cross-chain slippage update above received value", async function() {
        try {
            ...
            let ThousandAndOneTokens18 = ethers.utils.parseEther("1001");

            await BscStateHandler.updateState(BscStateHandlerCounter, [ThousandAndOneTokens18]);
        } catch (err) {
            /// should error;
            console.log(err)
    ...
    it("FTM=>BSC: cross-chain slippage update from unauthorized wallet", async function() {
        try {
            ...
            await BscStateHandler.connect(accounts[2]).updateState(BscStateHandlerCounter, [amount]);
        } catch (err) {
            /// should error;
            console.log(err)
            assert.include(err.message, "revert", 'AccessControl: account 0x3c44cdddb6a900fa2b585dd299e03d12fa4293bc is missing role 0x2030565476ef23eb21f6c1f68075f5a89b325631df98f5793acd3297f9b80123');
        }
    });
});