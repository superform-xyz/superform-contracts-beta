...
describe("strategies base tests:", async () => {
  ...
  describe("Strategies Infra Setup", async () => {
    before("Deploying BSC Venus Fork Infra", async function () {
      /// For AAVE do P0lygon
      await network.provider.request({
        ...
        params: [
          {
            forking: {
              chainId: process.env.PRODUCTION === "true" ? 56 : 97, // bsc
      ...
      // Deploying STRATEGY Vault
      const USDC_whale = "0xF977814e90dA44bFA03b6295A0616a897441aceC";
  
      await deployStrategy(
        "CompoundV2StrategyWrapper",
        ForkConfig.VENUS_BSC_COMPOUND.USDC,
        ForkConfig.VENUS_BSC_COMPOUND.XVS,
        ForkConfig.VENUS_BSC_COMPOUND.vUSDC,
        ForkConfig.VENUS_BSC_COMPOUND.Comptroller,
        USDC_whale
      );
    ...
    it("FTM=>BSC: user depositing to a BSC CompoundV2Wrapper", async function () {
      ...
      expect(await TokenImpersonated.balanceOf(BscDst.address)).to.equal(amount);
    ...
    it("FTM=>BSC: user depositing to a BSC CompoundV2Wrapper", async function () {
      ...
      expect(await TokenImpersonated.balanceOf(BscDst.address)).to.equal(0);
      expect(await TokenImpersonated.balanceOf(accounts[0].address)).to.equal(
        MilionTokensE18
      );
    ...
    it("FTM=>BSC: user withdrawing from BSC CompoundV2Wrapper", async function () {
      const vaultId = 1;
